...
while running:
    ...
    if result:
        ...
        points = [(outputlist[i]*640, outputlist[i+1]*480) for i in range(0, len(outputlist), 2)]
        #print(points)
        # display to screen
        screen.fill((0,0,0))
        #image_array_rgba = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2RGBA)
        #print(image_array.shape)
        screen.blit(pygame.image.fromstring(image_array.tobytes(), (640,480), "RGB"), pygame.Rect(0, 0, 100, 100))
        for point in points:
